    ...
    with gr.Blocks() as demo:
        ...
        with gr.Tab("Data Upload"):
            ...
            def process_upload(file):
                ...
                try:
                    ...
                    return df, df.copy(), shape, columns, dtypes_df, missing_df, df.head(), df.tail()
        ...
        with gr.Tab("Statistics"):
            ...
            def generate_statistics(df):
                ...
                # Categorical summary as DataFrame - show ALL unique values
                ...
                if cat_cols:
                    ...
                    for col in cat_cols:
                        ...
                        for value, count in value_counts.items():
                            pct = (count / len(df) * 100).round(2) if len(df) > 0 else 0
                            cat_data.append({
                                "Column": col,
                                "Value": value,
                                "Count": count,
        ...
        with gr.Tab("Visualizations"):
            ...
            # Update filter column choices when data loads
            ...
            # Update filter UI based on selected column
            def update_filter_ui(df, col):
                ...
                if pd.api.types.is_numeric_dtype(df[col]):
                    ...
                    return (
                        ...
                        gr.update(value=None, visible=False),
                        gr.update(value=None, visible=False),
                        gr.update(choices=unique_vals, value=[], visible=True),
                        gr.update(value="", visible=False),
                        gr.update(value="", visible=False),
                    )
            ...
            def apply_single_filter(df, filtered_df, col, min_val, max_val, cat_vals, d_start, d_end):
                ...
                # Start from original or already filtered
                ...
                try:
                    ...
                    return filtered_df, f"Error applying filter"
            ...
            # Update row count and preview when data loads
            ...
            def update_dropdowns_on_upload(df):
                ...
                return (
                    gr.update(choices=date_choices),  # ts_date_col
                    gr.update(choices=num_choices),   # ts_value_col
                    gr.update(choices=["None"] + cat_choices),  # ts_color_col
                    gr.update(choices=num_choices),   # dist_col
                    gr.update(choices=cat_choices),   # bar_cat_col
                    gr.update(choices=num_choices),   # bar_num_col
                    gr.update(choices=num_choices),   # scatter_x_col
                    gr.update(choices=num_choices),   # scatter_y_col
                    gr.update(choices=["None"] + cat_choices),  # scatter_color_col
                )
            ...
            def update_all_plots(df, ts_d, ts_v, ts_c, dist_c, bar_c, bar_n, bar_a, sc_x, sc_y, sc_c):
                ...
                # Time series with optional color grouping
                if ts_d and ts_v:
            ...
            def export_csv(df):
                if df is not None:
                    import tempfile
                    import os
                    # Create temp file with a nice name
        ...
        with gr.Tab("Insights"):
            ...
            def update_insight_dropdowns(df):
                ...
                return (
                    gr.update(choices=num_cols),  # insight_col
                    gr.update(choices=date_cols), # trend_date_col
                    gr.update(choices=num_cols),  # trend_value_col
                    gr.update(choices=num_cols),  # anomaly_col
                )
